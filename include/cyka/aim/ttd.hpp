...
[[nodiscard]] std::unordered_map<SteamId, std::vector<double>> computeTtd(
    const Samples& samples,
    const VisibilityBatch& vis,
    double max_lookback_s = DEFAULT_TTD_LOOKBACK_S);

/// Per-kill `ttd_ms` with the same lookback rule as `computeTtd`.
void attachKillTtd(Match& match,
                   const Samples& samples,
                   const VisibilityBatch& vis,
                   double max_lookback_s = DEFAULT_TTD_LOOKBACK_S);
